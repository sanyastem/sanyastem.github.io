...
tldr:
  - "Простое правило: Flexbox — для одного направления (строка или колонка), Grid — для двумерной сетки, где строки и колонки нужны одновременно."
  - "Типичная страница: Grid для макета (grid-template-columns: 260px 1fr) и сетки карточек, Flexbox — для навигации и содержимого карточек."
  - "Адаптивная сетка без медиазапросов: repeat(auto-fill, minmax(280px, 1fr)) сама считает, сколько колонок влезает по ширине."
  - "auto-fill резервирует пустые колонки и сохраняет размер ячеек, auto-fit растягивает элементы на всю ширину; для карточек обычно нужен auto-fill."
date: 2025-01-20
