...
Образ — это набор слоёв (layers). Каждая инструкция в Dockerfile создаёт новый слой. Слои кэшируются: если файл не изменился — Docker не пересобирает этот слой. (Если ты только начинаешь с Docker — сначала пройди [базовый гайд по контейнерам](/devops/docker-basics/).)
...
Главное правило: **то, что меняется реже — выше, что меняется чаще — ниже**. В CI это правило особенно окупается — см. [BuildKit cache в GitHub Actions](/devops/docker-buildkit-cache-github-actions/).
