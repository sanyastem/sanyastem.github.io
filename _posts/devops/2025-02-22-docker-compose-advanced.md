...
В реальных приложениях несколько сервисов: бэкенд, фронтенд, база данных, Redis, очередь задач. Запускать каждый вручную через `docker run` — неудобно. Compose описывает все сервисы в одном файле и запускает их одной командой. (Основы — в гайде [Docker для новичка](/devops/docker-basics/), образы — в статье про [Dockerfile](/devops/docker-dockerfile/).)
...
Не хардкоди секреты прямо в `docker-compose.yml`. Используй `.env` файл (и не забудь добавить его в [.gitignore](/git/gitignore/)):
