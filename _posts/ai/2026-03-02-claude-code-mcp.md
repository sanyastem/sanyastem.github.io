...
**MCP (Model Context Protocol)** — открытый протокол, который позволяет подключать внешние инструменты к Claude. Вместо того чтобы Claude знал только о файлах в твоём проекте, MCP-серверы дают ему доступ к базам данных, GitHub, браузеру, Jira, Slack и многому другому. (Базовая настройка Claude Code — в [первой статье серии](/ai/claude-code-setup/).)
...
Если нужен доступ к внутренней системе, можно написать свой сервер — MCP SDK доступен на TypeScript и Python (полный разбор с нуля — в статье [пишем свой MCP-сервер](/ai/claude-code-mcp-build/)):
