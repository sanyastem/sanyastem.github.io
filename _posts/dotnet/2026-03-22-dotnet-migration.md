...
howto:
  name: "Мигрировать проект с .NET Framework на .NET 10"
  totalTime: "PT8H"
  steps:
    - name: "Запустить Upgrade Assistant"
      text: "dotnet tool install -g upgrade-assistant; upgrade-assistant analyze MyProject.sln. Получишь отчёт о совместимости и предложениях по миграции."
    - name: "Обновить project files"
      text: "Старый .csproj (XML-формат с десятками строк) переписать в SDK-style (3-5 строк). Удалить packages.config — миграция на PackageReference."
    - name: "Перенести web.config → appsettings.json"
      text: "Connection strings, app settings — в appsettings.json + appsettings.Production.json. Logging, security settings — через ConfigureServices в Program.cs."
    - name: "Заменить Global.asax → Program.cs"
      text: "Application_Start, Application_Error → middleware pipeline через app.Use*. Areas, routes, filters настраиваются на app.MapControllers."
    - name: "Мигрировать EF6 → EF Core"
      text: "Заменить пакет EntityFramework на Microsoft.EntityFrameworkCore.SqlServer. Lazy loading требует .UseLazyLoadingProxies(). FluentAPI совместима, аннотации Data — частично."
    - name: "Прогнать unit-тесты + integration-тесты"
      text: "Обязательно покрытие до миграции. После каждого шага запускать тесты. WebFormsTests → WebApplicationFactory<T> для интеграционных."
    - name: "Деплой через Docker"
      text: ".NET 10 идеально живёт в Linux-контейнерах: mcr.microsoft.com/dotnet/aspnet:10.0 — образ 80 МБ. Multi-stage Dockerfile для уменьшения размера."
faq:
...
**.NET 10** вышел в ноябре 2025, это LTS-релиз с поддержкой до ноября 2028. После переезда откроются [фичи C# 12–14](/dotnet/csharp-modern-features/) и [современный EF Core](/dotnet/efcore-optimization/). Причины мигрировать:
...
> 💡 .NET Upgrade Assistant умеет автоматически конвертировать project files, web.config и часть кода. Запусти его первым — сэкономит несколько часов ручной работы. Ещё больше рутины снимает [Claude Code со скиллом /dotnet-migrate](/ai/claude-code-skills-migration/).
